...
def acceptance_probability(energy, new_energy, temp):
    ...
    if new_energy < energy:
        return 1.0
    return np.exp(float(energy - new_energy) / temp)


class tsp:
    """Travelling salesman solver for large sets where distance matrix isn't
    feasible, calculates neighborhoods and solves subproblems'"""
    def __init__(self, points, cycles, kmax = 3, max_tour = 0.0):
        self.cycles = cycles
        self.kmax = kmax
        ...
        self.dim = 40    # make 40 x 40 grid neighborhoods
        self.nsize = 3
        ...
        self._load_cached_path\
            ('/home/noah/class/discopt/tsp/testing/solutions/neighbor_33810.data')
        # self._initial_solution()
        # self._graphify()
        self.path_length = self._tour_length_path()
    ...
    def _tour_length_path(self):
        """Calculate length of current tour"""
        tour_length = 0.0
        for i in range(self.N):
            if i > 0:
                p1 = self.points[self.path[i]]
                p2 = self.points[self.path[i-1]]
                tour_length += length(p1, p2)
        tour_length += length(self.points[self.path[self.N-1]],
